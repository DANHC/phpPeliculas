...
class ClienteDAO {
    ...
    public function getClienteId() {
        $id = $this->db->select("SELECT max(id) AS id FROM Cliente");
        return $id['id'];
    }


    public function getClienteDetalleCompra($clienteId) {
        $sql = "SELECT * FROM vClienteCompraDetalle WHERE ClienteId = $clienteId";

        try {
            $res = $this->db->selectQuery($sql);
        } catch(PDOException $ex) {
            echo "Error al mostrar detalle: ".$ex->getMessage();
        }
